...
//     step[3].className = step[3].className.replace(" active", " complete");
//     showTab(0);
//     btnPrev.style.display = "none";
//     //btnNext.innerHTML = "Next";
//     currentTab = 0;
//     step[0].className = step[0].className.replace(" complete", " active");
//     for (var i = 1; i < step.length; i++) {
//         step[i].className = step[i].className.replace(" complete", " disable");
//     }
...
function showTab(n) {
    ...
    if(currentTab === 1) {
        ...
        document.querySelector('input[name="dateWedding"]').value = document.getElementById("dateWedding").value;
...
var validate = (id) => {
    ...
    if(id === "dateWedding") {
        ...
        if(table > reservedLobby.tableMax ||  table <= 0) {
...
var filter = () => {
    ...
    var newLobby = lobbies.map((lobby) => {
        ...
        return `<tr id='${lobby.idLobby}'><td>${lobby.nameLobby}</td><td>${lobby.typeLobby}</td><td>${lobby.tableMax}</td><td>${lobby.costMin}</td><td>${lobby.note}</td><td><div class="radio"><input type="radio" name="lobby" value=${lobby.idLobby}></div></td></tr>`;
    });
    //newLobby = newLobby.join('');
    //ko nen dung inner thi dung gi?
...
var updateReservedFood = (checkboxElem, id) => {
    ...
    let newReservedFood = reservedfood.map((food) => {
        return `<tr><td>${food.name}</td><td><input type="text" name="note" id='${food.id}note' value="${food.note}" onblur="updateNote(this,'${food.id}')"></td><td class="costFood">${food.cost}</td><td><input name="food" value='${food.id}' type="checkbox" id='${food.id}' onclick="updateReservedFood(this,'${food.id}')" checked></td></tr>`;
    });
    
    foods.innerHTML = newReservedFood.join('');
}

var updateNote = (note, id) => {
    for(let i=0; i<reservedfood.length; i++)
    {
        if(id == reservedfood[i].id) {
            reservedfood[i].note = note.value;
            return;
        }
    }
}
